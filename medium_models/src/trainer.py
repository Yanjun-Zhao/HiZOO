...
from src.lr_scheduler import zo_lr_scheduler
from src.Hessian_smooth_scheduler import Hessian_smooth_scheduler
...
class Trainer(LinearHeadTrainer):
    ...
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        ...
        """
        if self.args.hf_inference_model: #F
            return

        if self.optimizer is None: #T
            params = {}
            for n, p in self.model.named_parameters():
                if self.args.fix_layers > 0:#F
        ...
        if self.args.zo_lr_scheduler_type is None:
    ...
    def efficient_Hessian_perturb_parameters(self, model: nn.Module, random_seed, Hessian_matrix=None, scaling_factor=1):
        ...
        for name, param in self.named_parameters_to_optim:
            z_matrix = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            param.data = param.data + scaling_factor * torch.sqrt(Hessian_matrix[name]) * z_matrix * self.args.zero_order_eps
        return model
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        if self.args.fp16 and _use_apex:#F
            if not transformers.is_apex_available():
                raise ImportError("Please install apex from https://www.github.com/nvidia/apex to use fp16 training.")
            model, optimizer = amp.initialize(model, optimizer, opt_level=self.args.fp16_opt_level)

        # Multi-gpu training (should be after apex fp16 initialization)
        if self.args.n_gpu > 1:#F
            model = torch.nn.DataParallel(model)

        # Distributed training (should be after apex fp16 initialization)
        if self.args.local_rank != -1:#F
        ...
        # Train
        if transformers.is_torch_tpu_available():#F
        ...
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if self.args.zero_order_optim:
                    ...
                    if self.args.zo_by_layer:#F
                        ...
                        for _ in range(num_zs):
                            ...
                            with torch.no_grad():
                                ...
                                model = self.efficient_Hessian_perturb_parameters(model, seed, self.Hessian_matrix, scaling_factor=1)
                                loss1 = self.zo_forward(model, inputs)

                                # second function evaluation
                                model = self.efficient_Hessian_perturb_parameters(model, seed, self.Hessian_matrix, scaling_factor=-2)
                                loss2 = self.zo_forward(model, inputs)
                            ...
                            for name, param in self.named_parameters_to_optim:
                                z_matrix = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                                Hessian_temp = 1/self.Hessian_matrix[name]*z_matrix*z_matrix
                                #Hessian_temp = torch.abs(1/self.Hessian_matrix[name]*z_matrix*z_matrix - 1/self.Hessian_matrix[name])
                                Hessian_estimator=torch.abs(loss1+loss2-2*loss_original)* Hessian_temp /(2 * self.args.zero_order_eps*self.args.zero_order_eps)
                                self.Hessian_matrix[name]=(1-Hessian_smooth) * self.Hessian_matrix[name] + Hessian_smooth * Hessian_estimator
                                grad = (loss1-loss2)/(2 * self.args.zero_order_eps) * z_matrix * torch.sqrt(self.Hessian_matrix[name])
                            ...
                            if self.args.gradient_accumulation_steps > 1:#1
                            ...
                            # scale grad according to number of zs sampled
                            #if not self.args.scale_lr_with_samples:#F
                            #    projected_grad = projected_grad / float(num_zs)
                            ...
                            if self.args.zero_order_use_trainer_optim:#F
                                if self.args.efficient_zero_order:
                                    # print(random_seed)
                                    torch.manual_seed(random_seed)
                    ...
                    # if not using the trainer, the updates are resampled and directly applied to the parameters
                        ...
                        if (self.args.logging_steps > 0 and self.state.global_step % self.args.logging_steps == 0) or (
                                ...
                                logs = {}
                                logs["loss"] = loss_out.item()
                                logs["learning_rate"] = zo_learning_rate
                                logs["Hessian_smooth"] = Hessian_smooth
                                logs["global_step"] = self.state.global_step
                                logs["zo_forward_step"] = self.state.zo_forward_step
                                logs["max_steps"] = self.args.max_steps
                                logs["max_zo_forward_steps"] = self.args.max_zo_forward_steps
                                logs["time"] = int(time.time() - start_time)
                                logs["weight_decay"] = self.args.weight_decay
                                self.log(logs)
                                logger.info(str(logs))
                ...
                ###################################
                #if self.args.evaluate_during_training and self.state.global_step % 10 == 0:
            ...
            if self.args.max_steps > 0 and self.state.global_step > self.args.max_steps or (self.args.max_zo_forward_steps > 0 and self.state.zo_forward_step > self.args.max_zo_forward_steps):
            #if self.args.max_steps > 0 and self.state.global_step > 10 or (self.args.max_zo_forward_steps > 0 and self.state.zo_forward_step > self.args.max_zo_forward_steps):
