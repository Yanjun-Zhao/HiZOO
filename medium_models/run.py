...
class DynamicTrainingArguments(TrainingArguments):
    ...
    warmup_step: int = field(
        default=0,
        metadata={"help": "warmup"}
    )
    
    Hessian_batch: int = field(
        default=1,
        metadata={"help": "hessian batch"}
    )
    
    decay_step: int = field(
        default=0,
        metadata={"help": "warmup"}
    )
    zo_lr_scheduler_type: str = field(
        default='constant'
    )
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):#F
    ...
    if data_args.prompt:
        if data_args.prompt_path is not None:
            ...
            if data_args.template_path is not None:#F
            ...
            if data_args.mapping_path is not None:#F
    ...
    if data_args.auto_demo and model_args.few_shot_type == 'prompt-demo':#F
        # GPT-3's in-context learning
    ...
    if model_args.apply_lora:
        #pdb.set_trace()
        if 'roberta' in model_args.model_name_or_path:#T
    ...
    if training_args.hf_inference_model:#F
        ...
        #print lots of info of model weights not initialized form model ckpt ;newlt initial


    if training_args.tie_emb:#F
        logger.warn("Tie embeddings. Only work for RoBERTa (in our code by default they are not tied)")
        model.tie_emb()
    
    if training_args.head_tuning:#F
    ...
    if training_args.do_train:
        ...
        if training_args.trainer == "hessian":#F
        ...
        if training_args.trainer == "standard" or training_args.trainer == "linearhead":#F
            if training_args.save_at_last:#F
                trainer.save_model(training_args.output_dir)

            if trainer.is_world_process_zero():#T
                tokenizer.save_pretrained(training_args.output_dir)
                torch.save(model_args, os.path.join(training_args.output_dir, "model_args.bin"))
                torch.save(data_args, os.path.join(training_args.output_dir, "data_args.bin"))
            
            if training_args.evaluate_during_training:#T
